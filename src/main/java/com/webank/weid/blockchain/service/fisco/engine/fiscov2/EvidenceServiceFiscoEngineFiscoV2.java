 ...
 */
public class EvidenceServiceFiscoEngineFiscoV2 extends BaseEngineFisco implements EvidenceServiceEngineFisco {

    private static final Logger logger = LoggerFactory.getLogger(EvidenceServiceFiscoEngineFiscoV2.class);
     ...
     * @param groupId 群组编号
     */
    public EvidenceServiceFiscoEngineFiscoV2(String groupId) {
        super(groupId);
    ...
    private void initEvidenceAddress() {
        if (groupId == null || BaseServiceFisco.masterGroupId.equals(groupId)) {
            logger.info("[initEvidenceAddress] the groupId is master.");
            this.evidenceAddress = BaseServiceFisco.fiscoConfig.getEvidenceAddress();
            return;
        }
        this.evidenceAddress = super.getBucket(CnsType.ORG_CONFING).get(
            BaseServiceFisco.fiscoConfig.getCurrentOrgId(), WeIdConstant.CNS_EVIDENCE_ADDRESS + groupId).getResult();
